    ...
    try:
        ...
        async with DB_POOL.acquire() as connection:
            ...
            try:
                ...
                print(f"Notice: Could not ensure 'microproduct_name' column via ALTER: {alter_e}. This is okay if the column already exists.")
...
# CORRECTED ProjectApiResponse
class ProjectApiResponse(BaseModel):
    id: int  # <<<<<<<<<<< THIS IS THE CRUCIAL FIX
    projectName: str
    projectSlug: str
    product: str
    productSlug: str
...
class ProjectDB(BaseModel):
    ...
    microproduct_content: Optional[str] = None
    created_at: datetime
    model_config = {"from_attributes": True}
...
def transform_text_new_to_old(new_text: str, lang_configs: Dict = LANG_CONFIG) -> tuple[str, str]:
    # ... (keep as is, ensure it's the complete version from previous correct step) ...
    ...
    for module_match in module_pattern.finditer(new_text):
        ...
        lessons_block = module_match.group(4) # Group index depends on actual regex structure
...
def parse_training_plan_from_string(original_content_str: str, main_table_title: str) -> Optional[TrainingPlanDetails]:
    ...
    for line_num, line_raw in enumerate(lines, 1):
        ...
        if lesson_match and current_section_dict is not None:
            ...
            # Ensure lessons is a list before appending
    ...
    try:
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                ...
                project_data.microProductType, db_microproduct_name_to_store, project_data.aiResponse
            )
...
async def get_user_projects_from_db_transformed(onyx_user_id: str = Depends(get_current_onyx_user_id), pool: asyncpg.Pool = Depends(get_db_pool)):
    ...
    try:
        ...
        for r_dict_full in [dict(row) for row in db_rows]:
            ...
            if not type_for_slug_generation: type_for_slug_generation = "default-mptype-slug" # Fallback for slug generation
            ...
            pra = ProjectApiResponse(
                id=project_db_id, # This is where the ID is added
    ...
    try:
        ...
        for r_dict in [dict(r) for r in all_projects_raw]:
            ...
            if (db_project_slug == project_slug and
                ...
                found_project_row = r_dict
                break 
        
        if not found_project_row: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Micro-product not found.")

        project_name_from_db = found_project_row['project_name']
        product_type_from_db = found_project_row.get('product_type', 'N/A')
        microproduct_display_name = found_project_row.get('microproduct_name') or found_project_row.get('microproduct_type', 'N/A')
        microproduct_type_for_routing = found_project_row.get('microproduct_type') or "default-type"
        microproduct_content_str = found_project_row.get('microproduct_content')

        details_data: Optional[TrainingPlanDetails] = None
        detected_language_for_response = 'ru' 
        ...
        if not details_data:
             details_data = TrainingPlanDetails(mainTitle=f"Content for {microproduct_display_name}", sections=[], detectedLanguage=detected_language_for_response)
...
async def download_micro_product_pdf_from_db( document_slug: str, onyx_user_id: str = Depends(get_current_onyx_user_id), pool: asyncpg.Pool = Depends(get_db_pool)):
    ...
    if not target_row: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"PDF definition not found: {document_slug}")

    content_str = target_row.get('microproduct_content')
    details_pdf: Optional[TrainingPlanDetails] = parse_training_plan_from_string(content_str, project_name_for_pdf_context) if content_str else None
    ...
    if details_pdf and details_pdf.detectedLanguage:
        ...
        _, detected_lang_fallback = transform_text_new_to_old(content_str)
        detected_lang_for_pdf = detected_lang_fallback
        if details_pdf: 
            details_pdf.detectedLanguage = detected_lang_for_pdf
    ...
    try:
        ...
        if 'details' not in context_data_for_pdf:
             context_data_for_pdf = {'details': context_data_for_pdf if context_data_for_pdf else {}}
