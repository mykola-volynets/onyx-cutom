...
import { PipelineFormData, PipelineGetResponse, PROMPT_TEMPLATES, PromptTemplate } from '@/types/pipelines'; 
...
const EditPipelinePageComponent = () => {
      ...
      const fetchPipelineData = async () => {
        ...
        try {
          ...
          setFormData({
            ...
            discovery_prompts_list: data.discovery_prompts_list.length > 0 ? data.discovery_prompts_list : [''],
            structuring_prompts_list: data.structuring_prompts_list, 
          });
          setInitialPipelineName(data.pipeline_name);

          const initialSelectedTemplates = data.structuring_prompts_list.map(text => {
            return PROMPT_TEMPLATES.find(t => t.templateText === text) || null;
          });
          setSelectedStructuringTemplates(initialSelectedTemplates);
  ...
  const handleStructuringTemplateSelect = (index: number, templateId: string) => {
    ...
    if (selectedTemplate) {
      setFormData(prev => {
        const newList = [...prev.structuring_prompts_list]; newList[index] = selectedTemplate.templateText;
        return { ...prev, structuring_prompts_list: newList };
      });
      setSelectedStructuringTemplates(prev => {
        const newSelected = [...prev]; newSelected[index] = selectedTemplate; return newSelected;
      });
    } else {
        setFormData(prev => {
            const newList = [...prev.structuring_prompts_list]; newList[index] = "";
            return { ...prev, structuring_prompts_list: newList };
          });
        setSelectedStructuringTemplates(prev => {
            const newSelected = [...prev]; newSelected[index] = null; return newSelected;
          });
    }
  };
  ...
  const removeStructuringPromptSlot = (index: number) => {
    setFormData(prev => ({ ...prev, structuring_prompts_list: prev.structuring_prompts_list.filter((_, i) => i !== index) }));
  ...
  const moveStructuringPrompt = (index: number, direction: 'up' | 'down') => {
    setFormData(prevFormData => {
        const newList = [...prevFormData.structuring_prompts_list]; const item = newList[index];
        ...
        if (direction === 'up' && index > 0) { newList.splice(index, 1); newList.splice(index - 1, 0, item); newSelectedTemplates.splice(index, 1); newSelectedTemplates.splice(index - 1, 0, selectedItem); }
        else if (direction === 'down' && index < newList.length - 1) { newList.splice(index, 1); newList.splice(index + 1, 0, item); newSelectedTemplates.splice(index, 1); newSelectedTemplates.splice(index + 1, 0, selectedItem); }
  ...
  const renderStructuringPromptInputs = () => ( 
    <div className="pl-6 space-y-3 border-l-2 border-teal-200 py-2">
      ...
      {formData.structuring_prompts_list.map((promptText, index) => {
        const currentSelectedTemplateDetails = selectedStructuringTemplates[index];
        const selectedTemplateIdForDropdown = PROMPT_TEMPLATES.find(t => t.templateText === promptText)?.id || "";
          ...
          <div key={`structuring-${index}`} className="space-y-1 p-3 border border-gray-200 rounded-md bg-gray-50">
            <div className="flex items-start space-x-3">
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <select value={selectedTemplateIdForDropdown} onChange={(e) => handleStructuringTemplateSelect(index, e.target.value)} className="flex-grow text-sm p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 text-black bg-white">
                    <option value="" disabled>Select a template...</option>
                    {PROMPT_TEMPLATES.map(template => (<option key={template.id} value={template.id}>{template.name}</option>))}
                    </select>
                </div>
                {currentSelectedTemplateDetails && (
                    <div className="p-2 border rounded-md bg-white flex items-start gap-3">
                        {currentSelectedTemplateDetails.picture ? (<Image src={currentSelectedTemplateDetails.picture} alt={currentSelectedTemplateDetails.name} width={64} height={64} className="rounded object-cover w-16 h-16 flex-shrink-0" />)
                        ...
                        <div>
                            <p className="text-xs text-gray-700 font-semibold">{currentSelectedTemplateDetails.name}</p>
                            <p className="text-xs text-gray-500 mt-1 break-all">Text: &quot;{currentSelectedTemplateDetails.templateText.substring(0,100)}{currentSelectedTemplateDetails.templateText.length > 100 ? "..." : ""}&quot;</p>
                        </div>
                    </div>
                )}
                </div>
                <div className="flex flex-col space-y-1 items-center pt-1">
                <button type="button" onClick={() => moveStructuringPrompt(index, 'up')} disabled={index === 0} className={buttonBaseClasses} title="Move Up"><ArrowUpCircle size={18} /></button>
                <button type="button" onClick={() => moveStructuringPrompt(index, 'down')} disabled={index === formData.structuring_prompts_list.length - 1} className={buttonBaseClasses} title="Move Down"><ArrowDownCircle size={18} /></button>
                <button type="button" onClick={() => removeStructuringPromptSlot(index)} className={`${buttonBaseClasses} text-red-500 hover:text-red-700`} title="Remove Prompt"><Trash2 size={18} /></button>
                </div>
            </div>
          </div>);
      })}
  ...
  return (
    <main className="p-4 md:p-8 bg-gray-50 min-h-screen font-['Inter',_sans-serif]">
      <div className="max-w-3xl mx-auto bg-white p-6 md:p-8 shadow-lg rounded-lg border border-gray-200">
        ...
        {error && !isLoading && <div className="mb-4 p-3 bg-red-100 text-red-700 border border-red-300 rounded-md">{error}</div>}
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" disabled={isSubmitting || !formData.pipeline_name || isLoading} className="px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50">
