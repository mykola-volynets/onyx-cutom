...
export default function CourseOutlinePreview() {
  ...
  useEffect(() => {
    // On first mount call backend to generate preview using chat endpoint
  ...
  const handleGenerateFinal = async () => {
    ...
    try {
      const res = await fetch("/api/custom/projects/add", {
        ...
        body: JSON.stringify({
          projectName: prompt,
          design_template_id: 1, // TODO: Use real template id when available
