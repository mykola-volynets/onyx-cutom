...
const RenderBlock: React.FC<RenderBlockProps> = (props) => {
  const { block, depth = 0, isFirstInBox, isLastInBox, isMiniSectionHeadline, isListItemContent } = props;
  ...
  switch (block.type) {
    case 'headline': {
      ...
      if (isMiniSectionHeadline) {
          textStyleClass = level === 3 
            ? `text-base font-semibold ${THEME_COLORS.accentRed}` 
            : `text-sm font-medium ${THEME_COLORS.accentRed}`; 
          if (depth > 0) { 
            textStyleClass = level === 3 
                ? `text-sm font-semibold ${THEME_COLORS.accentRed}` 
                : `text-xs font-medium ${THEME_COLORS.accentRed}`;
          }
      }
      ...
      if (isMiniSectionHeadline && isFirstInBox) { 
        calculatedMb = 'mb-1'; 
      }
      ...
      return (
        <Tag
          ...
          style={{ backgroundColor: backgroundColor || 'transparent', color: headlineTextColor ? headlineTextColor : undefined , padding: backgroundColor ? '0.4rem 0.6rem' : undefined, borderRadius: backgroundColor ? '0.25rem' : undefined }}
        >
    ...
    case 'paragraph': { 
      ...
      if (isTopLevelParagraph) {
        // MODIFIED: Paragraph width changed to 55%
        paragraphClasses += ` w-[55%]`; 
      }

      const defaultMb = depth > 0 ? 'mb-1' : 'mb-2';
      const finalMb = isLastInBox ? 'mb-0' : defaultMb;
      paragraphClasses += ` ${finalMb}`;
      
      return (
        <p className={paragraphClasses.trim()}>
      ...
      return (
        <ListTag className={`list-none ${finalListMb} ${listSpacing}`}>
          {items.map((item, index) => {
            const itemNumber = isNumbered 
              ? `${depth > 0 ? String.fromCharCode(97 + depth -1) : ''}${index + 1}` 
              : '';
                ...
                <li key={index} className={`py-0.5 ${THEME_COLORS.primaryText} ${listItemBaseTextStyle}`}>
                  {typeof item === 'string' ? (
                    <div className="flex items-start">
                      <BulletIcon size={10} className={`mr-1.5 mt-[1px] shrink-0 ${THEME_COLORS.accentRed}`} /> 
                      <span>{item}</span>
                    </div>
                  ) : (
                    <RenderBlock 
                        block={item as AnyContentBlock} 
                        depth={depth + 1} 
                        isLastInBox={isLastInBox && isLastItemInCurrentList}
                    />
                  )}
                </li>
              );
    ...
    case 'alert': { 
        ...
        return (
          <div
            className={`p-2.5 border-l-4 rounded-r-sm ${finalBgColor} ${finalBorderColor} ${depth > 0 ? 'ml-3' : ''} ${finalAlertMb}`}
          ...
          >
            <div className="flex">
              <div className="shrink-0 pt-[1px]">
                <CustomIcon size={16} className={`${finalIconColorClass}`} style={{ color: finalIconColorStyle }} />
              </div>
              <div className="ml-2">
                {alertBlock.title && (
                  <h3 className={`${alertTextStyle} font-semibold ${finalTextColor}`} style={{ color: alertBlock.textColor || undefined }}>
                    {alertBlock.title}
                  </h3>
                )}
                <div className={`${alertTextStyle} ${finalTextColor} ${alertBlock.title ? 'mt-0.5' : ''}`} style={{ color: alertBlock.textColor || undefined }}>
                  {alertBlock.text}
                </div>
              </div>
            </div>
    ...
    case 'section_break': {
      ...
      if (sb.style === 'dashed') {
        hrClassName = "my-3 border-dashed border-gray-100";
      }
...
const PdfLessonDisplay = ({ data, lessonTime }: PdfLessonDisplayProps): React.JSX.Element | null => {
  ...
  while (i < data.contentBlocks.length) {
    if (data.contentBlocks[i].type === 'section_break') {
      i++; 
      continue;
    }
  ...
  return (
    <div className="font-['Inter',_sans-serif] bg-white p-4 sm:p-6 md:p-8 shadow-lg rounded-md max-w-3xl mx-auto my-6">
      {data.lessonTitle && (
        <div className={`mb-4`}> 
          {/* MODIFIED: Main title size reduced */}
          <h1 className={`text-2xl sm:text-3xl font-medium ${THEME_COLORS.headingText} mb-1.5`}>
            {data.lessonTitle}
          </h1>
          {lessonTime && (
            <div className={`flex items-center ${THEME_COLORS.mutedText} text-[10px] sm:text-xs mt-1`}>
        ...
        {renderableItems.map((item, index) => {
          ...
          if (index > 0) { 
            ...
            if (isSectionHeader(item)) {
              ...
              if (previousItem.type === 'mini_section') {
                prevItemExemptsDivider = true;
              } else if (previousItem.type === 'standalone_block' && previousItem.content.type === 'numbered_list') {
                prevItemExemptsDivider = true;
              }

              if (!prevItemExemptsDivider) {
                showDividerBeforeItem = true;
              }
            }
          }
          ...
          if (index === renderableItems.length - 1) {
            isCurrentItemEffectivelyLast = true;
          } else {
            const nextItem = renderableItems[index + 1];
            let dividerWillBeBeforeNextItem = false;
            if (isSectionHeader(nextItem)) { 
                let currentItemExemptsNextDivider = false;
                if (item.type === 'mini_section') {
                    currentItemExemptsNextDivider = true;
                } else if (item.type === 'standalone_block' && item.content.type === 'numbered_list') {
                    currentItemExemptsNextDivider = true;
                }
                if (!currentItemExemptsNextDivider) {
                    dividerWillBeBeforeNextItem = true;
                }
            }
            ...
            <React.Fragment key={fragmentKey}>
                      ...
                      if (typeof subItem === 'object' && 'type' in subItem && subItem.type === "mini_section") {
                        ...
                        return (
                          <div key={`major-${index}-mini-${subIndex}`} className={`${THEME_COLORS.veryLightAccentBg} border border-[#FF1414] rounded-lg p-3 my-2`}>
                            <RenderBlock block={miniSec.headline} isFirstInBox={true} isLastInBox={false} isMiniSectionHeadline={true} />
                            <RenderBlock block={miniSec.list} isFirstInBox={false} isLastInBox={true}/>
                          </div>
                        );
                      } else {
                        return <RenderBlock 
                                  key={`major-${index}-block-${subIndex}`} 
                                  block={subItem as AnyContentBlock} 
                                  isLastInBox={isLastSubItemInMajor && isCurrentItemEffectivelyLast} 
                                />;
                      }
                  })}
                </div>
              ...
              {item.type === "mini_section" && (
                <div className={`${THEME_COLORS.veryLightAccentBg} border border-[#FF1414] rounded-lg p-3 my-2`}>
                  <RenderBlock block={item.headline} isFirstInBox={true} isLastInBox={false} isMiniSectionHeadline={true} />
