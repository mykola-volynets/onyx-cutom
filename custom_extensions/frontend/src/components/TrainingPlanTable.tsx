...
import { TrainingPlanData, Section, Lesson, StatusInfo } from '@/types/trainingPlan';
...
const NewPracticeIcon = ({ color = '#FF1414', className = '' }) => (
  <svg className={className} width="16" height="16" viewBox="0 0 9 10" fill="none" xmlns="http://www.w3.org/2000/svg"> {/* Adjusted default size */}
...
const StatusBadge = ({ type, text }: { type: string; text: string }) => {
  const iconColor = '#FF1414'; // Default red for all icons now
  const defaultIconSize = "w-4 h-4"; // Tailwind class for approx 16px x 16px

  switch (type) {
    case 'no':
      return ( <div className="inline-flex items-center space-x-1.5"> <span className={`flex items-center justify-center text-white rounded-full ${defaultIconSize} shrink-0`} style={{ backgroundColor: iconColor }}> <X size={10} strokeWidth={3} /> </span> <span className="text-xs font-medium text-gray-700">{text}</span> </div> );
    case 'test':
    ...
    case 'practice': // Uses NewPracticeIcon
    ...
    case 'percent': // For contentAvailable percentages (e.g., "50%")
      return ( <div className="inline-flex items-center space-x-1"> <NewPieChartIcon color={iconColor} className={`${defaultIconSize} shrink-0`} /> <span className="text-xs font-medium text-gray-700">{text}</span> </div> );
    case 'none': // For contentAvailable = "Нет" - NOW USES RED PIECHART
       return ( <div className="inline-flex items-center space-x-1"> <NewPieChartIcon color={iconColor} className={`${defaultIconSize} shrink-0`} /> <span className="text-xs font-medium text-gray-700">{text}</span> </div> );
    default:
...
const TrainingPlanTable: React.FC<TrainingPlanTableProps> = ({ initialData }) => {
  const iconBaseColor = '#FF1414'; // Red for all clocks
  ...
  return (
    <div className="font-['Inter',_sans-serif] bg-gray-50"> {/* Removed padding and bg from here if parent has it */}
      <div className="shadow-lg rounded-lg overflow-hidden border border-gray-200 bg-white">
        ...
        <div className="grid grid-cols-10 gap-0 text-gray-500 p-4 text-xs font-semibold items-center border-b border-gray-200 uppercase tracking-wider">
            ...
            {/* Centered text for "Время" header */}
          ...
          {sections.map((section: Section) => (
            <React.Fragment key={section.id}>
               <div className="grid grid-cols-10 gap-0 bg-gray-100 p-4 font-semibold items-center border-t border-gray-200">
                  ...
                  {/* Module Header Clock: icon beside text, aligned like lesson rows */}
                  <div className="col-span-10 sm:col-span-1 flex items-center justify-start space-x-1 font-semibold px-2"> {/* Changed to justify-start for alignment */}
                      <div className="w-4 flex justify-center"> {/* Wrapper to center clock icon within its allocated space */}
                        <NewClockIcon color={iconBaseColor} className="w-4 h-4"/>
                      </div>
                      <span style={{ color: iconBaseColor }} className="flex-grow text-left">{section.totalHours}ч</span> {/* Text aligned left */}
                  </div>
               </div>
              {section.lessons.map((lesson: Lesson, lessonIndex: number) => (
                    <div key={lessonIndex} className="grid grid-cols-10 gap-0 p-4 items-center border-t border-gray-200 hover:bg-gray-50 transition-colors duration-150 min-h-[50px]">
                      ...
                      <div className="col-span-5 sm:col-span-2 flex justify-start px-2 border-r border-gray-300">
                          <StatusBadge type={lesson.check.type} text={lesson.check.text} />
                      </div>
                      <div className="col-span-5 sm:col-span-1 flex justify-start px-2 border-r border-gray-300">
                           <StatusBadge type={lesson.contentAvailable.type} text={lesson.contentAvailable.text} />
                      </div>
                      <div className="col-span-10 sm:col-span-2 text-gray-600 px-2 border-r border-gray-300">
                           {lesson.source}
                      </div>
                      {/* Lesson Time Column: Centered icon container, then text beside it */}
                      <div className="col-span-10 sm:col-span-1 flex items-center justify-start space-x-1 text-gray-500 px-2"> {/* Changed to justify-start */}
                         {/* Fixed width container for the icon to align all clocks */}
                         <div className="w-4 flex justify-center"> {/* Adjust w-4 if icon size changes */}
                            <NewClockIcon color={iconBaseColor} className="w-4 h-4" />
                         </div>
                         <span className="flex-grow text-left">{lesson.hours > 0 ? `${lesson.hours}ч` : '-'}</span> {/* Text aligned left */}
                      </div>
                    </div>
                ))}
            </React.Fragment>
